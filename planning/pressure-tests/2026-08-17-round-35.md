...
| `OP-R35-011` — Two pass/fail thresholds were "published" by nobody: the chain-walk time budget and the review-calibration floors. | `ISS-006` publishes the 1000-record golden budget (five seconds per walk on hosted runners); `ISS-041` publishes its floors (≥90% seeded-defect rejection per class, ≥80% known-good acceptance) with floor changes as reviewed replan decisions. |
| `OP-R35-012` — `ISS-039`/`ISS-041` asserted sequencing ("before X dispatches/relies") their blocks-nothing stance cannot enforce. | Both reworded: the divergence ledger and calibration report are declared inputs of the consuming issues' independent review packets; no phantom gates. |
