...
<bootstrap-custody-root>/state-mutation-destination-identities/<Dphys>/absence-observations/<Dabs>.json
...
| `Dabs`            | `external-destination-absence-observation/v1`                                                                                                               | `Dphys` raw32, `Ddest` raw32, destination-state-root digest raw32, absence-reason text, `Dobs` raw32, helper digest raw32, custody-instance digest raw32, observed-at text, canonical observation bytes                                                                                                                                        | `.../<Dphys>/absence-observations/<Dabs>.json`; create-once and read back in the same lock-held operation                                                      |
...
| `external-destination-absence-observation/v1`         | `custodyInstanceDigest:sha256`, `destinationDigest:sha256`, `helperDigest:sha256`, `locatorObservationDigest:sha256`, `observedAt:timestamp`, `physicalDestinationIdentityDigest:sha256`, `reason:RUNTIME_AUTHORITY_ABSENT\|DESTINATION_STATE_ROOT_ABSENT`, `schemaVersion:external-destination-absence-observation/v1`, `stateRootDigest:sha256`                                                                                                                                                                                                                                                                                 |

The physical leaf is exactly one decoded component. `leaf-bytes` must decode as
well-formed shortest-form UTF-8 with no BOM into a Unicode scalar sequence;
invalid UTF-8, surrogates, and noncharacters refuse. Normalization and
locale-independent full lowercase use pinned Unicode 15.1.0 normalization and
unconditional default-special-casing data. The canonical algorithm decodes,
lowercases only for a case-insensitive profile, normalizes, then UTF-8 encodes;
the resulting bytes must equal the decoded `leaf-bytes`. No uppercase or
case-fold round trip is applied, so canonical `straße` remains distinct from
`strasse` and valid.

The decoded component must contain neither `/` nor `\\`, `.` or `..`, NUL, a
drive or URI prefix, or a trailing dot/space. Windows additionally rejects
U+0000--U+001F, `" * / : < > ? \\ |`, alternate streams, and any ASCII-case-
insensitive basename before the first dot equal to `CON`, `PRN`, `AUX`, `NUL`,
`CLOCK$`, `COM1`--`COM9`, `LPT1`--`LPT9`, or the corresponding `COM`/`LPT`
superscript-1/2/3 aliases. Its selected profile is exactly
`DARWIN + CASE_INSENSITIVE_LOWERCASE + NFD`,
...
`Dabs` uses domain `external-destination-absence-observation/v1` and frames, in
order, `physicalDestinationIdentityDigest`, `destinationDigest`,
`stateRootDigest`, `reason`, `locatorObservationDigest`, `helperDigest`,
`custodyInstanceDigest`, `observedAt`, and canonical observation bytes. It is
produced only by the ISS-022-admitted helper while holding the applicable
destination and anchor locks, is create-once and read back before lock release,
...
bytes. For this archive, `observationDigest` equals the same-lock `Dabs`;
proposal `observationDigest` equals its current `ADMITTED` `Dobs`. The owner
mutation ID uses domain `destination-owner-mutation-id/v1` and
...
The proposal `positionDigest` uses domain `destination-owner-position/v1` and
frames only destination and the canonical owner-current path; it is stable for
the lifetime of that one physical destination owner.
...
uses domain `destination-owner-successor-review-candidate/v1` and frames, in
order, the outer `destinationDigest`, prior RETIRED tip/value/receipt,
`teardownArchiveDigest`, canonical `priorInstallation`, and canonical
`successorAuthority`. It excludes `candidateDigest`, the independent-review
record, `Dsrc`, successor anchor/owner selection, `Dsrp`, and all readbacks. The
authenticated `reviewReceiptDigest` is issued externally by the admitted review
path over that exact recomputed candidate and cannot be candidate-produced.
Prior installation
...
| `bootstrap-proposed-genesis-input/v1`                        | `authorityPathInstanceDigest:sha256`, `bootstrapGrantDigest:sha256`, `bootstrapTransactionId:uuid-v7`, `genesisPositionDigest:sha256`, `globalIdentityDigest:sha256`, `schemaVersion:bootstrap-proposed-genesis-input/v1`, `successorCoreDigest:sha256`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `state-mutation-bootstrap-anchor-lifecycle-archive/v1`       | `anchorDigest:sha256`, `archivedAt:timestamp`, `archivedReceiptDigest:sha256`, `archivedTipDigest:sha256`, `archivedValueDigest:sha256`, `destinationAbsenceDigest:sha256`, `lifecycle:ACTIVE\|CONSUMED`, `schemaVersion:state-mutation-bootstrap-anchor-lifecycle-archive/v1`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
archived tip/value/receipt, archived lifecycle, destination-absence digest, and
canonical archive bytes. `archivedAt` remains only inside canonical bytes. The
archive contains no teardown-receipt digest or future lifecycle value.
...
genesis input contains only the independent preimage facts needed to recompute
`Dsc` and genesis position: authority path, bootstrap grant and transaction,
`G`, and successor core. It contains no `useIntentDigest`-dependent `Dgb`,
GENESIS `Dh`, E0 `Dv`, or `Dbg`. The use-intent digest uses domain
...
canonical proposed-genesis input, canonical reviewed installer, canonical
reviewed helper, start, expiry, and canonical intent bytes. Only after that
digest exists may `Dgb`, GENESIS `Dh`, E0 `Dv`, and `Dbg` consume it and be
cross-checked downstream. The intent excludes every downstream digest,
proposal/receipt/tip/readback, and every CONSUMED record.
...
`destinationAbsenceDigest` equals a same-lock
`RUNTIME_AUTHORITY_ABSENT` `Dabs` over that exact `Dphys/Ddest`, state root,
current `Dobs`, helper, and custody.
...
custody, runtime value/proposal/tip readbacks, the distinct persisted Dgp
readback, owner ACTIVE
...
and canonical receipt bytes. `externalArchiveDigest` equals the recomputed
lifecycle-archive digest; `teardownEvidenceDigest` equals the same-lock `Dabs`
named by that archive. The receipt and archive are read back before the
RETIRED proposal is made.

The `<mutation-id>` path component for the consumption receipt is domain
`bootstrap-anchor-consumption-id/v1` over Dba, bootstrap transaction, Dgp, and
selected owner CONSUMED tip. The teardown receipt path component is domain
`bootstrap-anchor-teardown-id/v1` over Dba, prior anchor tip, retirement
transition, same-lock Dabs, and external lifecycle-archive digest. Both are
raw SHA-256 digests derived from the closed receipt inputs; timestamps and
readbacks are excluded. No caller supplies either path identity.
...
3. A create-once unexpired use intent equal-binds owner ACTIVE, anchor ACTIVE,
   transaction, grant, the independent `Dsc`/genesis-position preimage, helper,
   review, custody, and state root. Only downstream artifacts bind its digest
   into `Dgb/Dh/Dv/Dbg`. Expiry without such an intent permits only teardown
   after a same-lock `DESTINATION_STATE_ROOT_ABSENT` `Dabs`; a pre-expiry intent
   permits recovery only of its exact transaction.
4. E0 selects the precomputed value and immutable core, then `Dgp` binds its
   ...
   receipt.
5. Retirement creates and reads back the lifecycle archive over the selected
   prior anchor triple and same-lock `Dabs`, creates and reads back the teardown
   receipt whose `externalArchiveDigest` equals that archive digest, selects
   anchor RETIRED bound to the receipt, creates the owner teardown archive,
   then selects owner RETIRED bound to the anchor triple/archive. No receipt or
   archive names a future value or its selecting proposal/tip.
...
| anchor use intent  | schema `state-mutation-bootstrap-anchor-use-intent/v1`; domain `bootstrap-anchor-use-intent/v1`                   | `Dba` raw32, selected ACTIVE `Dbat` raw32, selected ACTIVE `Dbav` raw32, selected ACTIVE `Dbar` raw32, bootstrap transaction text, destination-state-root digest raw32, custody-instance digest raw32, independent proposed-genesis preimage canonical bytes, reviewed-installer/helper canonical bytes, started-at text, expires-at text, canonical use-intent bytes                                                                                                                                                                                                                                                                                                                                                                                                                     | `.../<installation>/use-intents/<transaction>.json`; `Dgb`/`Dh`/E0 `Dv`/`Dbg`, proposal/`Dr`/tip/`Dt`/readbacks excluded                                                       |
