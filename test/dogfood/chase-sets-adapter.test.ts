...
case "$*" in
  ...
  *workflows/platform-production.yml/runs*) printf '%s' '{"total_count":1,"workflow_runs":[{"id":42,"workflow_id":7,"head_sha":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","created_at":"2026-09-11T00:00:00Z"}]}' ;;
  *runs/42/attempts/1/jobs*) printf '%s' '{"total_count":1,"jobs":[{"id":43,"run_id":42,"run_attempt":1,"head_sha":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","name":"Deploy Staging","status":"completed","conclusion":"success","steps":${JSON.stringify(digestConclusion === "missing" ? [] : [{ name: "Verify immutable active release image", status: "completed", conclusion: digestConclusion }])}}]}' ;;
  *runs/42*) printf '%s' '{"id":42,"workflow_id":7,"head_sha":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","repository":{"full_name":"chase-sets/chase-sets"},"head_repository":{"full_name":"chase-sets/chase-sets"},"path":".github/workflows/platform-production.yml","run_attempt":1,"status":"completed","conclusion":"success"}' ;;
