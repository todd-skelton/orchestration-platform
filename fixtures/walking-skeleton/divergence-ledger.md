...
| Journal/restart       | A fixture-local retained-handle owner performs OPJ1 create-once, append, sync, read-back, strict parse and replay. The injection seam covers every event and owner boundary. Killed owners are not adopted: restart may read/reduce but must refuse `SESSION_HELD` before effects. |
