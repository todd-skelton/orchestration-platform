...
function planningSourceEffects(sourceText: string) {
  ...
  const argumentName = (index: number): string =>
    tokens[index + 2]?.kind === SyntaxKind.StringLiteral ? tokens[index + 2]!.value : "computed";
  ...
  for (const [index, token] of tokens.entries()) {
    ...
    if (token.text === "import") {
      if (next?.text === "(") findings.push(`dynamic-import:${argumentName(index)}`);
      else {
        const end = statementEnd(index);
        if (tokens.slice(index, end).some((candidate) => candidate.text === "="))
          findings.push("import-equals");
        const dependency = tokens
          .slice(index, end)
          .findLast((candidate) => candidate.kind === SyntaxKind.StringLiteral);
        if (dependency) dependencies.push(dependency.value);
      }
    }
    if (token.text === "export") {
      const end = statementEnd(index);
      const from = tokens.slice(index, end).findIndex((candidate) => candidate.text === "from");
      if (from !== -1) {
        const dependency = tokens
          .slice(index + from + 1, end)
          .find((candidate) => candidate.kind === SyntaxKind.StringLiteral);
        if (dependency) dependencies.push(dependency.value);
      }
    }
    if (token.text === "require")
      findings.push(next?.text === "(" ? `require:${argumentName(index)}` : "require:reference");
    if (token.kind === SyntaxKind.Identifier && ambientNames.has(token.value))
      findings.push(`ambient:${token.value}`);
    if (
      ...
      findings.push("ambient:Math.random");
  }
  return {
...
test("keeps quarantined planning source effect-free and outside manifest activation", async () => {
  ...
  expect(planningSourceEffects(sourceText)).toEqual({
    dependencies: ["../../../packages/contracts/src/index.js"],
    findings: [],
  });
  for (const mutant of [
    ...
    const inspected = planningSourceEffects(mutatedSource);
    expect(inspected).not.toEqual({
      dependencies: ["../../../packages/contracts/src/index.js"],
      findings: [],
    });
  }
